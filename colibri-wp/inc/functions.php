...
function colibri_theme_page_builder_components($components)
{
...
function colibri_theme_default_components($components)
{
	...
	$components = array_merge($components, array(
		...
		'navigation'     => "{$namespace}\\Header\\Navigation",
...
function colibri_theme_register_components($components = array())
{


	if (apply_filters('colibri_page_builder/installed', false)) {
		$components = colibri_theme_page_builder_components($components);
	} else {
		$components = colibri_theme_default_components($components);
	}

	return $components;
}

\ColibriWP\Theme\Core\Hooks::colibri_add_action('components', 'colibri_theme_register_components');
...
if (!apply_filters('colibri_page_builder/installed', false)) {
	colibri_assets()
		->registerTemplateScript(
			"colibri-theme",
			"/resources/theme/theme.js",
			array('jquery', 'jquery-effects-slide', 'jquery-effects-core')
		)
		->registerTemplateStyle("colibri-theme", "/resources/theme/theme.css")
...
add_action('colibri_page_builder/default_theme_data', 'colibri_theme_default_theme_data');
...
function colibriwp_print_archive_entry_class()
{
	global $wp_query;
	...
	if (colibriwp_is_wporg_preview()) {
		$hasBigClass = false;
	}

	if ($showBigEntry && $hasBigClass) {
		$classes[] = "h-col-sm-12 h-col-md-12";
	} else {
...
if (is_admin() && !is_customize_preview()) {
	require_once __DIR__ . '/class-wp-license-manager-client.php';
